...
void system_clock_init(void) {
    ...
    */
    ...
    RCC->PLLCFGR |= RCC_PLLCFGR_PLLSRC_HSE | (4U << RCC_PLLCFGR_PLLM_Pos) // M
...
void dwt_enable(void) {
    CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk; // enable trace subsystem (must be set or writes
                                                    // to DWT registers will be ignored)
...
static void tim2_setup_10ms(void) {
    ...
    TIM2->CR1 = 0; // clear
    TIM2->CR1 |= TIM_CR1_OPM; // one-pulse mode: counter stops counting at the next update event
                              // (clearing the bit CEN)

    TIM2->PSC = 8999; // Prescaler: The counter clock frequency (CK_CNT) is equal to fCK_PSC /
                      // (PSC[15:0] + 1) ==> 90 MHz / (8999+1) = 10 kHz
    TIM2->ARR =
            99; // Auto Reload: period = (ARR+1) ticks ==> (99+1) = 100 ticks @10 kHz => 10.000 ms
...
static void tim2_start_and_wait(void) {
    ...
    while ((TIM2->SR & TIM_SR_UIF) == 0)
        ; // spins until UIF (update interrupt flag) sets an overflow (i.e. after 10ms)
}
